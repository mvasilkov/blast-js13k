var perlin3 = function () {
    ...
    var grad3 = [
        new G(1, 1, 0), new G(-1, 1, 0), new G(1, -1, 0), new G(-1, -1, 0),
    ...
    function _seed(seed) {
        ...
        var i, k
        for (i = 0; i < 256; ++i) {
            k = p[i] ^ ((i & 1? seed: seed2) & 255)
            pr[i] = pr[i + 256] = k
            gp[i] = gp[i + 256] = grad3[k % 12]
        }
    }

    _seed(0|Math.random() * (1 << 16))

    function _fade(t) { return t * t * t * (t * (t * 6 - 15) + 10) }
    function _lerp(a, b, t) { return (1 - t) * a + t * b }
    ...
    return function (x, y, z) {
        ...
        var n000 = gp[X     + pr[Y     + pr[Z    ]]].dot3(x    , y    , z    );
        var n001 = gp[X     + pr[Y     + pr[Z + 1]]].dot3(x    , y    , z - 1);
        var n010 = gp[X     + pr[Y + 1 + pr[Z    ]]].dot3(x    , y - 1, z    );
        var n011 = gp[X     + pr[Y + 1 + pr[Z + 1]]].dot3(x    , y - 1, z - 1);
        var n100 = gp[X + 1 + pr[Y     + pr[Z    ]]].dot3(x - 1, y    , z    );
        var n101 = gp[X + 1 + pr[Y     + pr[Z + 1]]].dot3(x - 1, y    , z - 1);
        var n110 = gp[X + 1 + pr[Y + 1 + pr[Z    ]]].dot3(x - 1, y - 1, z    );
        var n111 = gp[X + 1 + pr[Y + 1 + pr[Z + 1]]].dot3(x - 1, y - 1, z - 1);
